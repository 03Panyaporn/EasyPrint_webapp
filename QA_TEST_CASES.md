...
| D04-03 | เปิดหน้าร้านที่ `pending`/`rejected` โดยตรงผ่าน URL/API | ไม่ควรเข้าถึงได้ (404) | API: `GET /shops/:id` ของร้าน pending และ rejected → `404 "ไม่พบร้านค้านี้"` ถูกต้องทั้งคู่ (ร้าน approved ควบคุม → `200`) — **แต่ UI พังหนักกว่าที่คาด**: guest เปิดหน้าร้านผ่าน browser (ทั้ง pending และ **แม้แต่ร้าน approved ก็ตาม**) ถูก redirect ไป `/login` ทันที ไม่เห็นหน้าร้านเลย → **BUG-04-01 (Critical)** | **FAIL (UI พังหนักกว่า scope เดิม)** |
...
| D04-05 | เปิดหน้าร้าน/บริการที่ไม่มีอยู่จริง (valid UUID แต่ไม่มีในระบบ / id รูปแบบผิด) | 404 สวยงาม ไม่ crash | valid-แต่ไม่มีจริง (`00000000-...`) → `404` ถูกต้อง; **id รูปแบบผิด (`/shops/not-a-uuid`) → `500`** แทน 400/404 — ยืนยันบั๊ก pattern เดียวกับที่เคยพบและแก้แล้วใน `addresses.ts` (BUG-02-02) แต่ยังไม่ถูกแก้ใน `shops.ts` → **BUG-04-02 (Medium)** | **FAIL (garbage id → 500)** |
