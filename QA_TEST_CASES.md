...
**สถานะ: ✅ เสร็จสมบูรณ์ (2026-09-08)** — ทดสอบผ่าน API ตรงด้วยร้านทดสอบใหม่หลายร้าน (ครบวงจร pending→approve/reject→suspend→reinstate→edit→delete) พบบั๊ก 1 จุด (Medium) — **แก้ไขและ verify แล้ว**

| ID | สถานการณ์ทดสอบ | ผลที่คาดหวัง | ผลจริง | Pass/Fail |
|---|---|---|---|---|
| AS13-01 | Approve/Reject ร้านที่สมัครใหม่ | สถานะเปลี่ยนถูกต้อง + notification ไปหาเจ้าของร้าน | สมัครร้านใหม่ 2 ร้าน → approve ร้านแรก (`pending→approved`, หายจาก pending list ในดชบอร์ด, เจ้าของร้านได้ notification ถูกต้อง); reject ร้านที่สอง โดยไม่กรอกเหตุผล → `400` ถูกต้อง, กรอกเหตุผล → `200` (`pending→rejected`) พร้อม `rejectedReason` บันทึกถูกต้อง | **PASS** |
| AS13-02 | Suspend พร้อมเหตุผล / ไม่กรอกเหตุผล | ตามเงื่อนไข | ไม่กรอกเหตุผล → `400` ถูกต้อง; กรอกเหตุผล → `200` (`approved→suspended`) บันทึก `rejectedReason` ถูกต้อง | **PASS** |
| AS13-03 | Reinstate ร้านที่ suspended | กลับ approved, ข้อความแจ้งเตือนควรต่างจาก approve ครั้งแรก (บั๊กเดิม A3-07) | **ยืนยันบั๊กจริงก่อนแก้:** reinstate (`suspended→approved`) ได้ข้อความ "ยินดีด้วย! ผ่านการตรวจสอบ..." **เหมือนเป๊ะ** กับตอนอนุมัติร้านสมัครใหม่ครั้งแรก ไม่แยกบริบทเลย → **BUG-13-01** → แก้แล้ว (เช็คสถานะก่อนอัปเดตว่าเป็น `suspended` ไหมก่อนเลือกข้อความ) → retest → reinstate ได้ข้อความ "การระงับการใช้งาน...ถูกยกเลิกแล้ว" ถูกต้อง แยกจากข้อความอนุมัติครั้งแรกชัดเจน; ทดสอบ regression ด้วยร้านสมัครใหม่จริง → ยัง approve ครั้งแรกได้ข้อความเดิมถูกต้อง ไม่กระทบ | **PASS** ✅ (หลังแก้ไข) |
| AS13-04 | แก้ไขข้อมูลร้าน (ชื่อ/ที่อยู่/ประเภทบริการ) | สำเร็จ | `PATCH /admin/shops/:id` แก้ชื่อ/เบอร์/ที่อยู่/ประเภทบริการ → `200` บันทึกถูกต้องครบทุกฟิลด์ | **PASS** |
| AS13-05 | ลบร้านที่ไม่มี/มี order-service ผูก | 200 / 409 (ไม่ใช่ 500) | ลบร้านทดสอบที่ไม่มี dependency → `200` สำเร็จ; สร้างร้านทดสอบใหม่ + ผูกบริการ 1 รายการ แล้วลองลบร้าน → `409` พร้อมข้อความสุภาพ ("ใช้การระงับแทนการลบ") ไม่ใช่ raw `500` — ลบบริการออกก่อนแล้วลบร้านซ้ำ → `200` สำเร็จ (ยืนยันว่า block เฉพาะตอนมี dependency จริง) | **PASS** |
| AS13-06 | Dashboard แสดงสถิติจริง (จำนวนร้าน/order/รายได้) | ตรงกับข้อมูลจริงใน DB | เทียบ `GET /admin/dashboard` กับ `GET /admin/shops` โดยตรง — `total`/`approved`/`pending` ตรงกันเป๊ะทุกตัวเลข; `pendingShops` list ตรงกับรายชื่อร้าน pending จริงทุกรายการ | **PASS** |

รายละเอียดเต็มดูที่ `QA_BUG_REPORT.md` (BUG-13-01)
