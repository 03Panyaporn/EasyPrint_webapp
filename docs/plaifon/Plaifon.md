...
### 35. Phase 1 ของระบบสั่งซื้อฝั่งลูกค้า — หน้ารายละเอียดร้าน + ระบบตะกร้า

ผู้ใช้ขอวางแผนระบบสั่งซื้อฝั่งลูกค้าทั้งหมด (login → เข้าดูร้าน → สั่งพิมพ์ → เพิ่มลงตะกร้า → checkout) — งานใหญ่มาก จึงวางแผนก่อนแล้วแบ่งเป็น phase โดยทำ **phase 1: หน้ารายละเอียดร้าน + ตะกร้า** ก่อน (เห็นผลเร็ว ทดสอบได้ทันที) ส่วน checkout/ระบบออเดอร์จริงเป็น phase ถัดไป

**สำรวจโค้ดก่อนวางแผน พบว่า:** ไม่มีหน้ารายละเอียดร้านฝั่งลูกค้าเลย, การ์ดร้านหน้าแรกกดไม่ได้อะไร, ไม่มีระบบตะกร้าอยู่เดิมเลยสักส่วน, หน้า `/orders`/`/orders/new` เป็นแค่โครงเปล่า, ฝั่งเว็บไม่เคยเช็คสถานะ login เลย (`getMe()` มีแต่ไม่ถูกเรียกใช้จริงที่ไหน), ตาราง `orders`/`createOrderSchema` เดิมเป็น placeholder ไม่เชื่อมกับระบบราคาจริงเลย (ต้องออกแบบใหม่ทั้งหมดใน phase ถัดไป)

**ถามยืนยัน 4 เรื่องก่อนทำ:** (1) ต้อง login ตอนไหน — ตอบ: ดูร้าน/บริการได้อิสระไม่ต้อง login เจอ prompt login แค่ตอนกด "เพิ่มลงตะกร้า" จริงๆ (2) ตะกร้าผสมร้านได้ไหม — ตอบ: 1 ตะกร้า = 1 ร้านเท่านั้น เหมือน food delivery (3) เก็บตะกร้าที่ไหน — ตอบ: DB ผูกกับ customerId (4) โหมดราคาตามพื้นที่ให้สั่งได้ใน phase นี้ไหม — ตอบ: ทำทั้ง 2 โหมดพร้อมกันเลย — และผู้ใช้ขอเพิ่มว่าต้องมี **ราคาโดยประมาณแบบเรียลไทม์** ตอนลูกค้าเลือก option/อัปโหลดไฟล์ด้วย

**สิ่งที่ทำ (backend):**
* ตาราง 3 ตารางใหม่: `carts` (unique `customer_id` — 1 คน 1 ตะกร้า, ผูก `shop_id` เดียว), `cart_items` (**ไม่เก็บราคาเลย** อ้างอิง `main_service_id` + `price_option_id` หรือ `area_color`/`area_width_cm`/`area_height_cm` ตาม pricingMode), `cart_item_addons` — migration บริสุทธิ์เพิ่มอย่างเดียว ไม่กระทบข้อมูลเดิม
* bucket ใหม่ `order-files` (private) บน Supabase Storage สำหรับไฟล์งานพิมพ์ที่ลูกค้าแนบ — รับ JPG/PNG/WEBP/PDF ไม่เกิน 20MB (ต่างจาก type รูปอื่นที่จำกัดแค่ 5MB) และบังคับ login เป็น customer ก่อนอัปโหลด (endpoint อื่นเปิดสาธารณะทั้งหมด แต่ type นี้มีแต่ลูกค้าที่ login แล้วต้องใช้อยู่แล้ว)
* เพิ่ม `GET /shops/:shopId` (สาธารณะ ร้านเดียว) — **เจอปัญหาระหว่างทำ:** ตอนแรกตั้งชื่อ param เป็น `:id` แล้ว Elysia/memoirist error ตอน compile route ทันที ("route already exists with a different parameter name") เพราะ router บังคับให้ทุก route ที่ path prefix ตรงกัน (`/shops/...`) ต้องใช้ชื่อ param เดียวกันทั้งหมด แม้ path ความยาวจะไม่เท่ากันก็ตาม แก้โดยเปลี่ยนเป็น `:shopId` ให้ตรงกับที่ไฟล์อื่นใช้อยู่แล้ว
* `POST/PATCH/DELETE /cart(/items/:id)` ครบ — **ราคาไม่เคยเก็บในตะกร้าเลย** `buildCartResponse()` คำนวณสดจาก `main_services`/`main_service_price_options`/`main_service_area_rates`/`main_service_addons` ทุกครั้งที่เรียก กันราคาไม่ตรงถ้าร้านแก้ราคาทีหลัง, เช็คความเป็นเจ้าของทุก endpoint ผ่าน join `cart_items`↔`carts`↔`customerId` กัน IDOR, เช็คว่า `priceOptionId`/`areaColor`/`addOnIds` ที่ส่งมาผูกกับ `mainServiceId`/ร้านนั้นจริงก่อนบันทึกเสมอ (กัน request ปลอมผสม ID ข้ามร้าน), เพิ่มของจากร้านอื่นทั้งที่มีของร้านเดิมค้างอยู่ได้ 409 พร้อม `conflictShopId` ให้ frontend ถามยืนยันก่อนล้างตะกร้าเดิม (`replaceCart: true`)
* **เจอบั๊กจริงระหว่างทำที่ต้องแก้เพิ่ม:** `GET /shops/:shopId/services` เดิมไม่คืน `id` ของแต่ละแถวใน `priceOptions` เลย (คืนแค่ `paperSize`/`color`/`price`) ทำให้ฝั่งลูกค้าไม่มีทางอ้างอิง priceOption ที่เลือกตอนเพิ่มลงตะกร้าได้ — แก้ backend ให้คืน `id` ด้วย (กระทบ POST/PATCH ที่ต้อง `.returning()` ค่าจริงจาก DB แทนการ echo ค่า input กลับไปเฉยๆ) เช่นเดียวกัน `cart_items.selection` ตอนแรกก็ไม่คืน `priceOptionId` (คืนแค่ paperSize/color ที่แสดงผล) ทำให้แก้ไขแค่จำนวนในตะกร้าไม่ได้เพราะไม่รู้จะส่ง priceOptionId ตัวไหนกลับไป — แก้ให้คืนด้วยเช่นกัน ทั้งสองจุดนี้ทดสอบจับได้จากการลองใช้งานจริงผ่านเบราว์เซอร์ ไม่ใช่แค่ curl
* ปิดช่องโหว่ open redirect ที่หน้า login — เพิ่ม `?redirect=` query param ให้ตอน login เสร็จพากลับไปหน้าที่ตั้งใจไว้ (เช่นหน้าร้านที่กำลังจะสั่งซื้อ) แต่ validate ว่าต้องเป็น path ภายในเว็บที่ขึ้นต้นด้วย `/` เดี่ยวเท่านั้น (ปฏิเสธ `//evil.com` ที่เบราว์เซอร์ตีความเป็น protocol-relative URL ออกนอกเว็บได้)

**สิ่งที่ทำ (frontend):**
* หน้าใหม่ `/shops/[shopId]` — รายละเอียดร้าน + list บริการหลักที่ active พร้อมปุ่ม "สั่งพิมพ์" เปิด modal ตั้งค่าออเดอร์ (`OrderBuilderModal.tsx`) — เลือกขนาด/สี (โหมด fixed) หรือกรอกกว้าง/สูงเอง (โหมด area) + เลือกบริการเสริม + จำนวน + อัปโหลดไฟล์ + โน้ต พร้อม **ราคาโดยประมาณคำนวณสดฝั่ง client ทุกครั้งที่แก้ค่าใดก็ตาม** (ใช้ข้อมูลเดียวกับที่ backend ใช้จริงเพราะเป็น public API เดียวกัน — แต่ตอนกดเพิ่มลงตะกร้าจริง backend คำนวณราคาซ้ำเองเสมอ ไม่เชื่อเลขที่ client โชว์)
* หน้าใหม่ `/cart` — list รายการ, ปรับจำนวน (+/-), ลบรายการ, เลือกวิธีจัดส่ง, ยอดรวมคำนวณจาก server จริงเสมอ (ไม่ใช่ query จาก state ที่ client แคชไว้) — ปุ่ม "ยืนยันสั่งซื้อ" ปิดไว้ก่อนพร้อมข้อความ "เร็วๆ นี้" เพราะ checkout เป็น phase ถัดไป
* หน้าแรก (`page.tsx`) — การ์ดร้านค้าตอนนี้กดแล้วพาไป `/shops/[shopId]` จริงแล้ว (เดิมกดไม่ได้อะไรเลย)
* Login required flow: กดเพิ่มลงตะกร้าตอนไม่ได้ login → ได้ 401 กลับมา → โมดัลเปลี่ยนเป็นข้อความแจ้งเตือนพร้อมปุ่ม "เข้าสู่ระบบ" ลิงก์ไปหน้า login พร้อม `?redirect=` กลับมาหน้าร้านเดิม — ทดสอบแล้วว่า login เสร็จพากลับมาหน้าเดิมถูกต้องจริง
* `/uploads` type ใหม่ `order-file`, `lib/api/cart.ts`, `lib/api/shops.ts` เพิ่ม `getShop()`, `packages/shared/src/schemas/cart.ts` validate การเลือกแบบ fixed/area แยกกันด้วย `superRefine` (คล้ายกับที่ใช้กับ pricingMode ของบริการหลักไปแล้วก่อนหน้านี้)

**ทดสอบจริงผ่านหน้าเว็บครบวงจร (ไม่ใช่แค่ curl):** สมัคร+อนุมัติร้านทดสอบ → สร้างบริการโหมด fixed (A4 ขาวดำ/สี) ผูกบริการเสริม + บริการโหมด area (ขาวดำ ฿80/สี ฿100 ต่อ ตร.ม.) → เปิดหน้า `/shops/[shopId]` จริงในเบราว์เซอร์ → เลือกสี+ติ๊กบริการเสริม+ใส่จำนวน 3 → **เห็นราคาประมาณ ฿105 อัปเดตสดถูกต้อง (=(5+30)×3)** → กดเพิ่มลงตะกร้าตอนยังไม่ login → **เจอ prompt login ถูกต้อง** → login ผ่าน redirect กลับมาหน้าร้านเดิมจริง → สั่งซื้อสำเร็จ → **เช็ค Supabase ว่า cart/cart_items/cart_item_addons บันทึกถูกต้องครบ** → เปิดหน้า `/cart` เห็นยอดรวม ฿70 ตรงกับที่คำนวณจริง → กด +/- จำนวนเปลี่ยนเป็น 3 ยอดอัปเดตเป็น ฿105 ถูกต้อง (พิสูจน์ว่าแก้ไขจำนวนในตะกร้าทำงานได้หลังแก้บั๊ก priceOptionId) → ลบรายการออกจากตะกร้าสำเร็จ → ทดสอบบริการโหมด area ด้วยตัวอย่างจริงจากผู้ใช้ "โปสเตอร์ 80×130 คิดเมตรละ 80 บาท" → **ราคาที่คำนวณได้ ฿83.2 ตรงตามสูตร (0.8×1.3×80) ทั้งฝั่ง client preview และ Supabase ที่บันทึกจริง** → ลบข้อมูลทดสอบทั้งหมดออกหลังเสร็จ

**ยังไม่ได้ทำ (phase ถัดไป):** `POST /cart/checkout` แปลงตะกร้าเป็นออเดอร์จริง, ออกแบบตาราง `orders`/`order_items`/`order_item_addons` ใหม่ให้เชื่อมกับระบบราคาจริง (ของเดิมเป็น placeholder), หน้าประวัติสั่งซื้อ (`/orders`, `/orders/[orderId]`) ที่ยังเป็นแค่โครงเปล่าอยู่, การแก้ไขขนาด/สีของรายการในตะกร้า (ตอนนี้แก้ได้แค่จำนวน — เปลี่ยนขนาด/สีต้องลบแล้วเพิ่มใหม่แทน)

### 36. รองรับหลายตะกร้าต่อลูกค้า (คนละร้านคนละใบ)

ผู้ใช้ถามหลัง phase 1 เสร็จว่า **"1 ลูกค้า หลายตะกร้าแต่ 1 ร้านได้ไหม"** — เดิมออกแบบไว้ว่า 1 ลูกค้ามีได้ตะกร้าเดียว ผูกกับร้านเดียว ถ้าเพิ่มของร้านอื่นต้องยืนยันล้างตะกร้าเดิมก่อน (409 + `replaceCart`) เปลี่ยนมาเป็น **1 ลูกค้ามีได้หลายตะกร้าพร้อมกัน แต่ 1 ตะกร้ายังผูกกับ 1 ร้านเท่านั้น** (ยังห้ามผสมสินค้าจากหลายร้านในตะกร้าเดียว) — ลูกค้าช้อปจากร้าน A และร้าน B พร้อมกันได้โดยไม่ต้องเลือกล้างตะกร้าฝั่งไหนทิ้ง

**สิ่งที่ทำ (backend):**
* Schema: เปลี่ยน unique constraint บน `carts` จาก `unique(customer_id)` เป็น composite `unique(customer_id, shop_id)` — migration แยก 2 ไฟล์ (`0009` ตารางเดิม, `0010` ลบ/เพิ่ม constraint) เพื่อเลี่ยง prompt แบบ interactive ของ `drizzle-kit generate` ตอนเจอการเปลี่ยน constraint
* เขียน `apps/api/src/routes/cart.ts` ใหม่ทั้งหมดให้เป็น shop-scoped: `GET /carts` (ตะกร้าทั้งหมดของลูกค้า), `GET /shops/:shopId/cart` (ตะกร้าร้านเดียว), `PATCH/DELETE /shops/:shopId/cart` (จัดส่ง/ล้างตะกร้าเฉพาะร้านนั้น) — เอา logic เช็ค conflict/409/`replaceCart` ออกทั้งหมดเพราะไม่มี conflict ให้เช็คอีกต่อไป (`POST .../cart/items` แค่ find-or-create ตะกร้าตาม `(customerId, shopId)` ตรงๆ)
* ตัด field `replaceCart` ออกจาก `packages/shared/src/schemas/cart.ts` ทั้งหมด (ไม่มีความหมายอีกต่อไป)

**สิ่งที่ทำ (frontend):**
* `lib/api/cart.ts`: เปลี่ยน `getCart()`/`clearCart()` เดิม (global ไม่ระบุร้าน) เป็น `getCarts()` (คืน `Cart[]` ทุกร้าน) และ `getShopCart(shopId)`/`clearShopCart(shopId)`/`setCartDeliveryOption(shopId, input)` (ระบุร้านเสมอ)
* `OrderBuilderModal.tsx`: ลบ flow ยืนยันล้างตะกร้าเดิม (`errors.conflict` branch + `handleReplaceCart`) ทิ้งทั้งหมด เหลือ submit ทางเดียว
* `/shops/[shopId]` badge ตะกร้าเปลี่ยนไปเรียก `getShopCart(shopId)` แทน `getCart()` เดิม
* เขียนหน้า `/cart` ใหม่ให้ลูปแสดงตะกร้าทีละร้าน (`carts.map(...)`) แทนตะกร้าเดียว — ปรับจำนวน/ลบรายการ/เลือกวิธีจัดส่งแต่ละช่องอ้างอิง `cart.shopId` ของตัวเองเสมอ ไม่ปนกับร้านอื่นในหน้าเดียวกัน

**ทดสอบจริงผ่านหน้าเว็บ:** login ลูกค้าทดสอบ → เพิ่มสินค้าจากร้าน "Johan Printer" (ปริ้นสี A4 · สี) → เพิ่มสินค้าจากร้าน "TONFAH PRINTER" (ถ่ายเอกสารขาวดำ A4 · ขาวดำ) → เปิด `/cart` เห็น **2 การ์ดแยกกันของ 2 ร้านพร้อมกันจริง** พร้อมยอดรวมถูกต้องแยกร้าน → กด +1 จำนวนใน Johan Printer (2 → ฿4) **TONFAH ไม่เปลี่ยนแปลงเลย** → ลบรายการเดียวใน Johan Printer ออก **การ์ดร้านนั้นหายไปทั้งการ์ด ส่วน TONFAH ยังอยู่ครบ** → เช็คตรงใน Supabase (`carts`/`cart_items`) ยืนยันว่ามี 2 แถวใน `carts` (คนละ `shop_id` เดียวกัน `customer_id`) ตรงกับที่ออกแบบไว้ → ลบข้อมูลทดสอบ (แถว carts ทั้งหมดของลูกค้าทดสอบ + ไฟล์ที่อัปโหลดใน bucket `order-files`) ออกหลังเสร็จ

### 37. หน้าสั่งพิมพ์เต็มหน้าจอ + โหมดราคาตามจำนวนหน้า PDF + พรีวิวไฟล์ (แทน OrderBuilderModal เดิม)

ผู้ใช้ส่งรูปตัวอย่างหน้าสั่งพิมพ์ของคู่แข่ง (อัปโหลด PDF + พรีวิวเอกสารแบบมี thumbnail/page นับหน้า + สรุปค่าใช้จ่าย) บอกว่า **"ไม่ใช่ หน้าสั่งซื้อลูกค้าไม่ใช่ให้เลือกแบบที่มีหลายอันแบบนี้ ... ให้มีเหมือนอยู่ในโค้ดก่อนหน้านี้"** — ตรวจสอบทั้ง repo/git history แล้วไม่พบโค้ดแบบนี้อยู่จริง (สิ่งที่ใกล้เคียงที่สุดคือ `PdfViewerLightbox.tsx` ฝั่งร้านดูออเดอร์ ซึ่ง mock ทั้งหมด ไม่ได้อ่าน PDF จริง) จึงวางแผนใหม่ทั้งหมดแล้วถามยืนยันก่อนทำ

**ยืนยัน 4 เรื่องก่อนทำ:** (1) ราคาต่อหน้าเป็นโหมดราคาใหม่ที่ 3 แยกจาก fixed/area (2) นับจำนวนหน้าฝั่ง client ด้วย pdf.js เพื่อ UX เรียลไทม์ (3) โปสเตอร์/พื้นที่กำหนดเองใช้ระบบ fixed/area เดิมตรงๆ ไม่เพิ่ม config ใหม่ (4) ทำเป็นหน้าเต็มแยก (`/shops/[shopId]/order/[serviceId]`) แทน modal เดิม — ผู้ใช้ตอบเพิ่มว่าโหมด area/โปสเตอร์ต้องมีพรีวิวด้วย คำนวณสัดส่วน ถ้าไฟล์ไม่พอดีขนาดที่กรอกให้เตือน+แนะนำ ถามว่าควรใช้ AI ช่วยแนะนำไหม — ตอบว่า **ไม่ต้องใช้ AI จริง** เพราะคำนวณด้วยคณิตศาสตร์ล้วน (เทียบอัตราส่วนไฟล์ vs กรอบ, คำนวณ DPI = พิกเซล/(ซม./2.54)) ได้แม่นยำและไม่มีต้นทุน/เวลาแฝง ผู้ใช้เห็นด้วย เลือกแนวทาง crop-preview + ปุ่มปรับขนาดอัตโนมัติ (aspect mismatch) และเตือน+แนะนำขนาดสูงสุด (DPI ต่ำ)

**สิ่งที่ทำ (schema/backend):**
* เพิ่ม `pricingMode` ค่าที่ 3 `"per_page"`, ตารางใหม่ `main_service_page_rates` (color + pricePerPage คู่กัน เหมือน area_rates), `cart_items` เพิ่ม column `pageCount` — migration `0011`
* **กฎความปลอดภัยสำคัญ**: `apps/api/src/routes/cart.ts` เพิ่ม `countPdfPages()` ใช้ `pdf-lib` ดาวน์โหลดไฟล์จาก Supabase Storage แล้วนับหน้าเองฝั่ง server เสมอ (ตอน `POST`/`PATCH` cart item โหมด per_page) **ไม่มี field ให้ client ส่ง pageCount มาเลยในสคีมา** — ป้องกัน customer แก้ตัวเลขจำนวนหน้าใน request เพื่อกดราคาถูกลง (หลักการเดียวกับที่ห้ามเชื่อราคา/อัตราจาก client ทุกโหมดราคาที่ผ่านมา)
* `apps/api/src/routes/services.ts` เพิ่ม CRUD `pageRates` คู่ขนานกับ `areaRates` เดิมทุกจุด (serializer, POST, PATCH, DELETE, fetchPageRates)

**สิ่งที่ทำ (frontend):**
* `AddServiceModal.tsx` เพิ่มตัวเลือกที่ 3 "ราคาต่อหน้า (PDF)" พร้อม UI ตั้งอัตราต่อหน้าแยกสี (คู่ขนานกับ UI โหมด area เดิม)
* สร้างหน้าใหม่ `/shops/[shopId]/order/[serviceId]` (มือถือ-first, layout 2 คอลัมน์บนจอใหญ่/สแตกบนมือถือ พร้อมแถบราคา+ปุ่ม sticky ด้านล่างจอสำหรับมือถือ) แทนที่ `OrderBuilderModal.tsx` เดิมทั้งหมด (ลบไฟล์ทิ้ง) — รองรับทั้ง 3 โหมดราคาในหน้าเดียว:
  * โหมด `per_page`: ใช้ `pdfjs-dist` (dynamic import ฝั่ง client เท่านั้น กัน SSR พัง) เรนเดอร์พรีวิวหน้า PDF จริง + ปุ่มเปลี่ยนหน้า + นับจำนวนหน้า + ตรวจจับขนาดกระดาษ (เทียบ pt→mm กับ A3/A4/A5/Letter) แสดงเป็นข้อมูลประกอบ ราคาที่โชว์เป็นแค่ preview ฝั่ง client เท่านั้น
  * โหมด `area`: พรีวิวไฟล์ (รูปหรือหน้าแรกของ PDF) ในกรอบตามสัดส่วนกว้าง/สูงที่กรอก (CSS `object-cover` แสดงผลลัพธ์การ crop จริงในตัว) + คำนวณ mismatch สัดส่วน (>2% เตือน + ปุ่ม "ปรับขนาดให้พอดีกับไฟล์อัตโนมัติ") + คำนวณ DPI (ต่ำกว่า 150 เตือน + แนะนำขนาดสูงสุดจากความละเอียดไฟล์จริง)
  * ปุ่ม "รีวิว" มุมขวาบน เปิด popup **placeholder ว่างไว้ก่อน** (ยังไม่มีระบบรีวิวจริง) ตามที่ผู้ใช้ขอ
* **เจอปัญหาระหว่าง build:** `next build` ล้มเพราะ Terser แปลง `import.meta` ในไฟล์ `pdf.worker.min.mjs` ของ pdfjs-dist ไม่ได้ (ไฟล์ worker ถูกดึงเข้า bundle ผ่าน `new URL(...)`) — แก้โดยก็อปไฟล์ worker ไปไว้ที่ `apps/web/public/pdf.worker.min.mjs` ตรงๆ แล้วตั้ง `workerSrc = "/pdf.worker.min.mjs"` แทน (asset สาธารณะ ไม่ผ่าน webpack/Terser เลย)

**ทดสอบจริงผ่านหน้าเว็บ (โหมด per_page):** ร้านค้าสร้างบริการ "ราคาต่อหน้า" อัตราขาวดำ ฿3/หน้า, สี ฿5/หน้า ผ่านหน้าเว็บจริง → login ลูกค้า เปิดหน้าสั่งพิมพ์ → สร้างไฟล์ PDF จริง 5 หน้า (ด้วย `pdf-lib` ในสคริปต์ทดสอบ) อัปโหลดจริง → **เห็นพรีวิวหน้า PDF จริง เปลี่ยนหน้าได้ นับได้ 5 หน้าถูกต้อง ตรวจพบขนาด A4 ถูกต้อง** → ราคาประมาณอัปเดตเป็น ฿15 (5×3) ตรง → ปรับจำนวนเป็น 2 ราคาขึ้นเป็น ฿30 ตรง → กดเพิ่มลงตะกร้า → **เช็ค Supabase ตรงว่า server นับหน้าเองได้ 5 หน้าจริง (ไม่ได้เชื่อจาก client) และ `/cart` แสดงผล "ขาวดำ · 5 หน้า" ฿30 ถูกต้อง** → **เจอบั๊กระหว่างทดสอบ**: pdfjs-dist อ่านขนาดกระดาษผิดเป็น "Letter" แทน "A4" เฉพาะไฟล์ PDF ที่ใช้ compressed object streams (ฟีเจอร์ PDF 1.5+) — เป็นปัญหาเฉพาะการแสดงผลป้ายขนาดกระดาษ (ไม่กระทบการนับจำนวนหน้า/ราคาเลย) ยืนยันด้วยไฟล์ PDF แบบ classic xref ว่าอ่านขนาดถูกต้อง (A4 210×297มม.) — บันทึกไว้เป็นข้อจำกัดที่ทราบแล้ว ไม่ได้แก้เพิ่มเพราะไม่กระทบราคา/ธุรกิจ → ลบข้อมูลทดสอบทั้งหมด (บริการ, ตะกร้า, ไฟล์ใน Storage) ออกหลังเสร็จ

**ข้อจำกัดที่ทราบแล้ว:** ป้ายขนาดกระดาษที่ตรวจพบ (โหมด per_page) อาจแสดงผิดพลาดสำหรับไฟล์ PDF บางแบบที่ใช้ compressed object streams เป็นแค่จุดแสดงผลข้อมูลประกอบ ไม่กระทบราคาที่คำนวณจริงเพราะคำนวณจาก page count (นับหน้าถูกต้องเสมอ) ไม่ใช่จากขนาดกระดาษ

### 38. รื้อระบบราคาใหม่ทั้งหมด — จาก fixed/area/per_page hardcode เป็นระบบ "ตัวเลือกบริการ" (Options) ที่ร้านค้าสร้างเองได้ไม่จำกัด

ในวันเดียวกันหลังทำ section 37 เสร็จ ผู้ใช้ส่งสเปกยาวมา (หัวข้อ "Improve Store Service Configuration UX") ขอรื้อระบบตั้งค่าบริการหลัก/บริการเสริมใหม่ทั้งหมด ให้ร้านค้าทั่วไปสร้างบริการเองได้โดยไม่ต้องมีความรู้ด้านเทคนิค รองรับตัวเลือกแบบ dropdown/radio/checkbox/number/text ที่สร้างเองได้ไม่จำกัด (แทนที่จะ hardcode "ขนาดกระดาษ"/"สี" ตายตัวแบบเดิม) — เป็นการเปลี่ยนสถาปัตยกรรมใหญ่ที่แทนที่ระบบ `fixed`/`area`/`per_page` ที่เพิ่งสร้างเสร็จใน section ก่อนหน้าทั้งหมด

**ถามยืนยัน 3 เรื่องก่อนทำ (งานใหญ่ ไม่อยากทำผิด):** (1) ระบบ Options ใหม่แทนที่ระบบ pricingMode เดิมทั้งหมดเลยไหม หรือเป็นชั้นเสริม — ตอบ **แทนที่ทั้งหมด** (2) ขอบเขตใหญ่มาก ควรแบ่ง phase ไหม — ตอบ **แบ่ง phase** (backend/schema ก่อน → shop-side builder UI → customer-side dynamic rendering) (3) ข้อมูลบริการเดิมที่ตั้งไว้แล้ว (ร้านทดสอบ) จะหายไหมถ้ารีเซ็ต — ตอบชัดเจนว่า **เฉพาะ `main_services`/ราคาที่ตั้งไว้เท่านั้นที่หาย ร้านค้า (`shops`) และบัญชีผู้ใช้ยังอยู่ครบ** เพราะเป็น dev database ยังไม่มีลูกค้าจริง จึงตัดสินใจรีเซ็ตแทนเขียน migration แปลงข้อมูลเก่า (ประหยัดเวลา ไม่มีความเสี่ยงจริง)

**ออกแบบสูตรราคาใหม่ (ตัดสินใจเอง เพราะสเปกไม่ได้ระบุสูตรคำนวณตรงๆ):** `pricingModel` (per_page/per_piece/per_sqm/fixed) กำหนดแค่ "หน่วย" ที่ `basePrice` จะถูกคูณด้วย ส่วน "ตัวเลือก" (options) เป็นราคาบวกเพิ่มแบบ flat ต่อการเลือก 1 ครั้ง (ไม่ใช่ต่อหน่วย) — สูตรรวม: `(basePrice + ผลรวม extraPrice ของตัวเลือกที่เลือก) x หน่วยตาม pricingModel x quantity` แล้วบวก add-on ที่เลือก — ตรงกับตัวอย่างในสเปก (เช่น A3 +10, สี +5 เป็นค่าคงที่ ไม่ใช่ต่อหน้า)

**สิ่งที่ทำ (schema — migration `0012`):**
* เพิ่ม enum `pricing_model` (`per_page`/`per_piece`/`per_sqm`/`fixed`) และ `service_option_type` (`dropdown`/`radio`/`checkbox`/`number`/`text`)
* `main_services` เพิ่ม column `pricing_model`, `base_price`, `requires_file_upload`, `allowed_file_types` — ลบ column `pricing_mode` เดิมทิ้ง
* ตารางใหม่ `service_options` (ชื่อ+type+sortOrder) และ `service_option_values` (ชื่อ+extraPrice+sortOrder ผูกกับ option) — **ลบตาราง `main_service_price_options`/`main_service_area_rates`/`main_service_page_rates` ทิ้งทั้งหมด**
* `cart_items` ลบ column `price_option_id`/`area_color`/`area_width_cm`/`area_height_cm` เดิม เพิ่ม `width_cm`/`height_cm` (ใช้ตอน per_sqm) — `page_count` เดิมยังคงไว้ (ใช้ตอน per_page เหมือนเดิม)
* ตารางใหม่ `cart_item_option_selections` (PK ร่วม `cart_item_id`+`option_id` — 1 รายการมีได้แค่ 1 ค่าต่อ 1 ตัวเลือก) เก็บ `value_id` (โหมด dropdown/radio/checkbox) หรือ `text_value` (โหมด number/text)
* **เจอปัญหาระหว่างสร้าง migration:** `drizzle-kit generate` ปกติเจอ interactive prompt ถามว่า "rename หรือ create ใหม่" (เพราะ enum/table/column ใหม่คล้ายของเดิมที่กำลังถูกลบ) และ pipe `\n` เข้า stdin ธรรมดาไม่ทำงานกับ prompt แบบนี้ (เป็น raw TTY reader ไม่ใช่ readline) — แก้โดย pipe คีย์ `\r` (carriage return) หลายตัวสลับกับ `sleep 0.5` ในลูป (`for i in $(seq 1 20); do printf '\r'; sleep 0.5; done | drizzle-kit generate`) ให้แต่ละ prompt มีเวลาขึ้นก่อนรับ input ทีละตัว — เลือก "create ใหม่" (ตัวเลือกแรก/default) ทุกจุดสำเร็จ ได้ SQL ที่ถูกต้อง (ต้องแก้ลำดับ statement เองนิดเดียว — ย้าย `ALTER TABLE cart_items DROP CONSTRAINT ...price_option_id...` มาไว้ก่อน `DROP TABLE main_service_price_options` เพราะ FK ต้องถูกลบก่อนตารางที่มันอ้างอิงถึง)

**สิ่งที่ทำ (backend):**
* `apps/api/src/routes/services.ts` เขียนใหม่ทั้งหมด — `writeOptions()`/`fetchOptions()` แทนที่ `writeAreaRates`/`writePriceOptions` เดิม, เพิ่ม endpoint ใหม่ `POST /shops/:shopId/services/:id/duplicate` (คัดลอกบริการพร้อม options/values/addon binding ทั้งชุด ตั้งชื่อใหม่อัตโนมัติ เช่น "... (คัดลอก)", "... (คัดลอก 2)" ถ้าชื่อชนกัน และ **ปิดใช้งานไว้ก่อนเสมอ** กันลูกค้าเห็นบริการซ้ำก่อนร้านตรวจสอบ ตามที่สเปกขอ)
* `apps/api/src/routes/cart.ts` เขียนใหม่ทั้งหมด — `validateOptionSelections()` เช็ค optionId ผูกกับ mainServiceId จริง + บังคับกรอกตาม type (dropdown/radio/number บังคับ, checkbox/text ไม่บังคับ) + valueId มีอยู่จริง, `writeOptionSelections()` เขียน selections ใหม่ทั้งชุดแบบ replace, `buildCartResponse()` คำนวณราคาตามสูตรใหม่ (basePrice + ผลรวม extraPrice ของ option ที่เลือก แล้วคูณหน่วยตาม pricingModel แล้วคูณ quantity)

**สิ่งที่ทำ (frontend):**
* `AddServiceModal.tsx` เขียนใหม่ทั้งหมด — เพิ่ม `OptionEditor` sub-component จัดการ draft state ของแต่ละตัวเลือก (ชื่อ+type+ค่าย่อย) เอง, Pricing Model เป็น radio 4 แบบพร้อม label ภาษาอังกฤษตามสเปก, ช่องตั้งค่า `requiresFileUpload`+`allowedFileTypes` (PDF/JPG/PNG/AI/PSD), ปุ่ม "เพิ่มตัวเลือก" ไม่จำกัดจำนวน, เพิ่ม dropdown "ข้อความสำเร็จรูป" (งานเอกสาร/โปสเตอร์/ป้ายไวนิล/นามบัตร/สติ๊กเกอร์) เติมช่องคำอธิบายให้อัตโนมัติแล้วแก้ต่อได้ตามสเปกข้อ "ลดการพิมพ์ข้อมูลซ้ำ"
* `MainServicesTable.tsx` เพิ่มปุ่ม "คัดลอกบริการ" (เรียก endpoint duplicate ใหม่), คอลัมน์ราคาเปลี่ยนมาโชว์ `basePrice` + label ตาม pricingModel + จำนวนตัวเลือกเสริม
* หน้า `/shops/[shopId]/order/[serviceId]` (ที่เพิ่งสร้างใน section 37) แก้ไขให้ render ฟอร์มแบบ dynamic ตาม `mainService.options[]` แทนการ hardcode fixed/area/per_page เดิม — dropdown/radio เป็นปุ่มชิปเลือก 1 ค่า, checkbox เป็น toggle เดี่ยว, number/text เป็นช่องกรอกอิสระ — โค้ด PDF preview/page count (per_page) และ crop-preview/DPI warning (per_sqm) จาก section 37 ใช้ซ้ำได้เกือบทั้งหมด แค่เปลี่ยนเงื่อนไข gate จาก `pricingMode` เป็น `pricingModel`
* `/cart` เปลี่ยนจากแสดง `item.selection` (fixed/area/per_page แบบเดิม) เป็นแสดง `item.optionSelections[]` (generic) + `item.unitBreakdown` (per_page/per_sqm) — PATCH แก้จำนวนต้องส่ง `optionSelections` กลับไปครบชุดเหมือนโหมดเดิมที่ต้อง resend state เต็มทุกครั้ง

**ทดสอบจริงผ่านหน้าเว็บครบวงจร:** ร้านค้าสร้างบริการทดสอบโหมด `per_piece` basePrice ฿20 พร้อม 5 ตัวเลือกครบทุก type (ประเภทกระดาษ dropdown A4/+0,A3/+10 — สี radio ขาวดำ/+0,สี/+5 — เข้าเล่ม checkbox เข้าเล่มสันกาว/+15 — จำนวนหน้าที่ต้องการเพิ่ม number — ข้อความสลัก text) + ผูกบริการเสริม (+30) ผ่านหน้าเว็บจริง (ขับเคลื่อน form ผ่าน synthetic DOM events เพราะฟอร์มมี field แบบไดนามิกเยอะ) → **เช็ค Supabase ตรงว่าทุก option/value บันทึกถูกต้องครบ** → login ลูกค้า เปิดหน้าสั่งพิมพ์ → **เห็นตัวเลือกทั้ง 5 render ถูก type ครบ** → เลือก A3(+10)+สี(+5)+ติ๊กเข้าเล่ม(+15)+กรอกเลข 3+กรอกข้อความ+อัปโหลดไฟล์+ติ๊ก add-on(+30)+จำนวน 2 ชุด → **ราคาประมาณอัปเดตเป็น ฿160 ตรงตามสูตร ((20+10+5+15+30)×2)** → กดเพิ่มลงตะกร้า → **เช็ค Supabase ยืนยันว่า `cart_item_option_selections` บันทึกครบทั้ง 5 ตัวเลือกถูกต้อง (รวม textValue ของ number/text)** → เปิด `/cart` เห็นรายการ options ครบ ยอดรวม ฿160 ตรง → กด +1 จำนวนเป็น 3 ยอดอัปเดตเป็น ฿240 ถูกต้อง (พิสูจน์ว่า PATCH resend optionSelections ทำงานถูก) → ลบข้อมูลทดสอบทั้งหมด (บริการ, ตะกร้า, ไฟล์ Storage) ออกหลังเสร็จ

**ยังไม่ได้ทำ (นอกขอบเขตรอบนี้ ตามที่สเปกไม่ได้บังคับ/ผู้ใช้ไม่ได้ขอเพิ่ม):** Safety UX แบบ custom dialog (ใช้ `window.confirm()` เดิมแทน ซึ่งทำหน้าที่เดียวกัน), tooltip อธิบายตัวเลือกให้ลูกค้า (มีแค่ชื่อ+ราคาเพิ่ม), field "บังคับกรอก" แยกสำหรับ type `text` (สเปกบอกว่าเป็นของอนาคต ไม่ต้องทำตอนนี้), การแจ้งเตือนอัตโนมัติเมื่อแก้ไข/ลบบริการเสริมที่ผูกกับบริการหลักอยู่ (ตอนนี้ backend cascade ลบให้เฉยๆ ไม่มี warning พิเศษ)
