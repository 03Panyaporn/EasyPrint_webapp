...
export default function CustomerContactAdminForm() {
  ...
  const [subject, setSubject] = useState("สอบถามคำสั่งซื้อ");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await submitCustomerContactMessage({ subject, message });
      setSuccess(true);
      setSubject("สอบถามคำสั่งซื้อ");
      setMessage("");
    } catch (err) {
        ...
        <div className="p-6 md:p-8">
          {success && (
            <div className="mb-6 bg-green-50 text-green-700 p-4 rounded-xl flex items-start gap-3 border border-green-200">
              ...
              <div>
                ...
                <p className="text-sm text-green-600 mt-1">ทีมงานได้รับข้อความของคุณแล้ว และจะติดต่อกลับโดยเร็วที่สุด</p>
              </div>
            </div>
          )}
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              ...
              >
                <option value="สอบถามคำสั่งซื้อ">สอบถามคำสั่งซื้อ</option>
                <option value="ปัญหาการชำระเงิน">ปัญหาการชำระเงิน / การคืนเงิน</option>
                <option value="แจ้งปัญหาร้านค้า">แจ้งปัญหาเกี่ยวกับร้านค้า</option>
